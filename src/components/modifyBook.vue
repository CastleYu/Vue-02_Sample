<template>
    <el-dialog v-bind="$attrs" v-on="$listeners" @open="onOpen" @close="onClose" :show-close=false
        ...
        <div slot="footer" style="margin: 0;">
            ...
            <el-button type="primary" @click="handleConfirm" :disabled="progressing"> <template
                    v-if="!progressing">确定</template>
                <template v-else><i class="el-icon-check" /></template></el-button>
        </div>
    </el-dialog>
</template>
